...
APP_VERSION = "AI_FIX_TRENDYOL_FORCE_V4_2026-05-23"
...
ads_df, dbg_ads = load_ad_spend_summary()
...
with st.sidebar:
    ...
    include_creative_spend = st.checkbox("Kreatif raporu harcamasını finans toplamına dahil et", value=False, help="Normalde kapalı kalsın. Aynı Meta harcaması iki kez sayılmasın diye kapalıdır.")
    show_debug_top = st.checkbox("Üstte debug özeti göster", value=False)

if include_creative_spend and not creative_df.empty:
    total_ad_spend += float(creative_df["spend"].sum())
    total_ad_revenue += float(creative_df["attributed_revenue"].sum())
    roas = safe_divide(total_ad_revenue, total_ad_spend)
    net_profit_proxy = total_revenue - total_ad_spend
    cac = safe_divide(total_ad_spend, known_orders)
...
st.info("Kreatif raporu finans toplamına otomatik eklenmez. Böylece Meta harcaması, Shopify/Trendyol reklam dosyalarıyla iki kez sayılmaz.")
