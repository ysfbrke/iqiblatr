...
AI_VERSION = "AI_RESTORED_TOTAL_REPORTING_ADSPEND_V6_2026-05-23"
...
with st.sidebar:
    ...
    include_creative_in_finance = st.checkbox(
        "Meta Spend için Kreatif raporunu öncelikli kullan",
        value=True,
        help="Açık kalırsa Meta Spend, Kreatif/Meta raporundaki harcama toplamından alınır. Kapalıysa Shopify/Meta billing veya Meta performance dosyaları yedek kaynak olarak kullanılır.",
    )
...
def build_financial_ads(raw_ads: pd.DataFrame, creative_df: pd.DataFrame, use_creative_meta: bool = True) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Finans toplamında reklam harcaması artık sadece şu iki ana kalemden oluşur:
      1) Trendyol reklam harcaması: manual_weekly_trendyol_ads.csv weekly_spend
      2) Meta Spend: önce Kreatif/Meta raporu, yoksa Shopify/Meta billing-performance dosyaları

    Bu fonksiyon, eski total raporlama ve canlı AI yapısını bozmadan sadece finansa girecek
    reklam datasını temiz ve denetlenebilir hale getirir.
    """
    base_cols = ["platform", "date", "spend", "attributed_revenue", "campaign_name", "source_file", "spend_source"]
    if raw_ads is None or raw_ads.empty:
        raw_ads = pd.DataFrame(columns=base_cols)
    else:
        raw_ads = raw_ads.copy()
        for c in base_cols:
            if c not in raw_ads.columns:
                raw_ads[c] = "" if c not in ["date", "spend", "attributed_revenue"] else (pd.NaT if c == "date" else 0.0)

    # 1) Trendyol reklam harcaması: sadece manual weekly dosya.
    spend_source_norm = raw_ads.get("spend_source", pd.Series(dtype=str)).astype(str).str.lower()
    source_file_norm = raw_ads.get("source_file", pd.Series(dtype=str)).astype(str).str.lower()
    platform_norm = raw_ads.get("platform", pd.Series(dtype=str)).astype(str).str.lower()
    trendyol_mask = (
        platform_norm.eq("trendyol")
        & (spend_source_norm.str.contains("manual weekly trendyol", na=False) | source_file_norm.str.contains("manual_weekly_trendyol_ads", na=False))
    )
    trendyol_finance = raw_ads[trendyol_mask].copy()
    if not trendyol_finance.empty:
        trendyol_finance["platform"] = "Trendyol"
        trendyol_finance["spend_source"] = "Trendyol Reklam Harcaması"

    # 2) Meta Spend: öncelik kreatif/metadan gelen spend.
    meta_finance = pd.DataFrame(columns=base_cols)
    meta_source_label = "Meta Spend"
    if use_creative_meta and creative_df is not None and not creative_df.empty and "spend" in creative_df.columns:
        tmp = creative_df.copy()
        if "report_date" not in tmp.columns:
            tmp["report_date"] = pd.NaT
        if "attributed_revenue" not in tmp.columns:
            tmp["attributed_revenue"] = 0.0
        if "campaign_name" not in tmp.columns:
            tmp["campaign_name"] = "Meta/Kreatif"
        if "source_file" not in tmp.columns:
            tmp["source_file"] = "creative_report"
        meta_finance = pd.DataFrame({
            "platform": "Meta",
            "date": tmp["report_date"],
            "spend": pd.to_numeric(tmp["spend"], errors="coerce").fillna(0.0),
            "attributed_revenue": pd.to_numeric(tmp["attributed_revenue"], errors="coerce").fillna(0.0),
            "campaign_name": tmp["campaign_name"].astype(str),
            "source_file": tmp["source_file"].astype(str),
            "spend_source": "Meta Spend - Kreatif Raporu",
        })
        meta_source_label = "Meta Spend - Kreatif Raporu"
    else:
        # Kreatif yoksa veya kapalıysa Shopify/Meta/Billing gibi dosyaları yedek kabul et.
        meta_mask = (
            spend_source_norm.str.contains("meta", na=False)
            | source_file_norm.str.contains("meta|facebook|fb|billing|campaign", na=False, regex=True)
        ) & ~trendyol_mask
        meta_finance = raw_ads[meta_mask].copy()
        if not meta_finance.empty:
            meta_finance["platform"] = "Meta"
            meta_finance["spend_source"] = "Meta Spend - Dosya/Billing"
            meta_source_label = "Meta Spend - Dosya/Billing"

    finance_ads = pd.concat([trendyol_finance, meta_finance], ignore_index=True)
    if not finance_ads.empty:
        finance_ads["spend"] = pd.to_numeric(finance_ads["spend"], errors="coerce").fillna(0.0)
        finance_ads["attributed_revenue"] = pd.to_numeric(finance_ads["attributed_revenue"], errors="coerce").fillna(0.0)
        finance_ads["date"] = pd.to_datetime(finance_ads["date"], errors="coerce")

    trendyol_spend = float(trendyol_finance["spend"].sum()) if not trendyol_finance.empty else 0.0
    trendyol_revenue = float(trendyol_finance["attributed_revenue"].sum()) if not trendyol_finance.empty else 0.0
    meta_spend = float(meta_finance["spend"].sum()) if not meta_finance.empty else 0.0
    meta_revenue = float(meta_finance["attributed_revenue"].sum()) if not meta_finance.empty else 0.0
    breakdown = pd.DataFrame([
        ...
        {"Kaynak": "Meta Spend", "Tutar": meta_spend, "Reklam Cirosu": meta_revenue, "Açıklama": meta_source_label},
...
finance_ads, ad_spend_breakdown = build_financial_ads(ads, creative, include_creative_in_finance)
...
lines_revenue_by_platform = (
    lines.groupby("platform")["line_revenue"].sum()
...
ad_spend = float(finance_ads["spend"].sum()) if not finance_ads.empty else 0.0
ad_revenue = float(finance_ads["attributed_revenue"].sum()) if not finance_ads.empty else 0.0
...
valid_metric_dates = []
for df, col in [(orders, "order_date"), (finance_ads, "date")]:
    if not df.empty and col in df.columns:
...
if not finance_ads.empty:
    ads_by_platform = finance_ads.groupby("platform", as_index=False).agg(
        ad_spend=("spend", "sum"),
...
with k3:
    st.metric("Hedef Gerçekleşme", pct(target_rate))
...
with tab2:
    ...
    if platform_summary.empty:
        ...
        st.dataframe(platform_summary.sort_values("net_profit_after_ads", ascending=False).style.format({
            ...
            "order_net_sales_reference": "{:,.2f} TL", "line_revenue_reference": "{:,.2f} TL",
...
with tab4:
    ...
    with c1:
        ...
        st.markdown("##### Finans Toplamına Giren Detay Satırlar")
        if finance_ads.empty:
            st.info("Finans toplamına dahil reklam verisi bulunamadı.")
        else:
            ad_view = finance_ads.groupby(["platform", "spend_source", "campaign_name"], as_index=False).agg(spend=("spend", "sum"), attributed_revenue=("attributed_revenue", "sum"))
...
with tab5:
    ...
    st.markdown("#### Reklam Harcaması Toplam Kontrolü")
    st.dataframe(ad_spend_breakdown.style.format({"Tutar": "{:,.2f} TL", "Reklam Cirosu": "{:,.2f} TL"}), use_container_width=True, hide_index=True)
    st.caption(f"Yapay Zeka Reklam Harcaması = Trendyol reklam harcaması + Meta Spend = {money(ad_spend)}")
    ...
    st.markdown(
        ...
        **Yeni mantık:**
        ...
        - Reklam Harcaması finans hesabında sadece Trendyol reklam harcaması + Meta Spend olarak hesaplanır.
        - Meta Spend için öncelik Kreatif/Meta raporundadır; yoksa Shopify/Meta billing-performance dosyaları yedek kaynak olur.
