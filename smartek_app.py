...
def to_float(value) -> float:
    ...
    if isinstance(value, (int, float)):
        return float(value)
    s = str(value).strip()
    s = s.replace("TL","").replace("TRY","").replace("₺","").replace("%","").replace('"',"").replace("\xa0","").replace(" ","")
...
def save_uploads(platform: str, kind: str, files):
    if files is not None:
        st.session_state[f"{platform}_{kind}_files"] = [{"name": f.name, "bytes": f.getvalue()} for f in files]
...
def files_from_state(platform: str, kind: str):
    ...
    for item in st.session_state.get(f"{platform}_{kind}_files", []):
        ...
        obj = UploadedObj()
        obj.name = item["name"]
        obj.getvalue = lambda b=item["bytes"]: b
        out.append(obj)
    return out
...
def upload_boxes(platform: str):
    ...
    st.warning("Manuel giriş kaldırıldı. Satış, maliyet ve reklam dosyalarını ayrı kutulara yükle.")
    c1,c2,c3 = st.columns(3)
    with c1:
        sales = st.file_uploader(f"{platform} satış / sipariş dosyaları", type=["csv","xlsx","xls"], accept_multiple_files=True, key=f"u_{platform}_sales")
        if sales: save_uploads(platform, "sales", sales)
    with c2:
        cost = st.file_uploader(f"{platform} maliyet dosyaları", type=["csv","xlsx","xls"], accept_multiple_files=True, key=f"u_{platform}_cost")
        if cost: save_uploads(platform, "cost", cost)
    with c3:
        ads = st.file_uploader(f"{platform} reklam harcaması dosyaları", type=["csv","xlsx","xls"], accept_multiple_files=True, key=f"u_{platform}_ads")
        if ads: save_uploads(platform, "ads", ads)
    loaded = {"Satış": len(st.session_state.get(f"{platform}_sales_files", [])), "Maliyet": len(st.session_state.get(f"{platform}_cost_files", [])), "Reklam": len(st.session_state.get(f"{platform}_ads_files", []))}
...
def load_market_sales(platform: str):
    frames, debug = [], []
    for f in files_from_state(platform, "sales"):
        ...
        revenue_col = find_col(df, ["Faturalanacak Tutar","Net Satış Tutarı","Net Satis Tutari","Satış Tutarı","Satis Tutari","Ürün Tutarı","Urun Tutari","Sipariş Tutarı","Siparis Tutari","Toplam Satış Tutarı","Toplam Satis Tutari","Mağazanın Brüt Cirosu","Magazanin Brut Cirosu","Toplam Brüt Ciro","Toplam Brut Ciro","Ciro","Tutar","Amount","Revenue"])
...
def platform_data(platform: str):
    ...
    total_revenue = orders["net_sales"].sum() if not orders.empty else 0.0
    order_count = orders["order_count"].sum() if not orders.empty and "order_count" in orders else 0.0
    units_sold = lines["qty"].sum() if not lines.empty else 0.0
    gross_profit = lines["gross_profit"].sum() if not lines.empty else 0.0
    ad_spend = ads["ad_spend"].sum() if not ads.empty and "ad_spend" in ads else 0.0
    ...
    return metrics, orders, lines, ads, debug

def kpi_cards(metrics: dict, show_ad_revenue: bool = True):
    fields = [("Total Revenue","total_revenue","money"), ("Order Count","order_count","int"), ("Units Sold","units_sold","int"), ("AOV","aov","money"), ("Gross Profit Before Ads","gross_profit_before_ads","money"), ("Total Ad Spend","total_ad_spend","money")]
    if show_ad_revenue:
        fields.append(("Total Ad Revenue","total_ad_revenue","money"))
    fields += [("ROAS","roas","ratio"), ("Net Profit After Ads","net_profit_after_ads","money"), ("MER","mer","ratio"), ("Cost Match Rate","cost_match_rate","percent")]
    for i in range(0, len(fields), 4):
        cols = st.columns(4)
        for col, (label, key, typ) in zip(cols, fields[i:i+4]):
            v = metrics.get(key, 0.0)
            if typ == "money": col.metric(label, money(v))
            elif typ == "int": col.metric(label, f"{v:,.0f}")
            elif typ == "ratio": col.metric(label, f"{v:.2f}" if v else "N/A")
            elif typ == "percent": col.metric(label, f"{v:.1%}" if v else "0.0%")

def show_platform(platform: str):
    st.header(platform)
    upload_boxes(platform)
    metrics, orders, lines, ads, debug = platform_data(platform)
    ...
    t1, t2, t3, t4 = st.tabs(["Satış", "Ürün & Kâr", "Reklam", "Debug"])
    with t1:
        if orders.empty:
